...
def pca(argv):
    ...
    assert(len(result) == len(index_to_id))
    result = [(index_to_id[i], result[i]) for i in range(len(index_to_id))]
    result.sort()
    for name, components in result:
        print '%s\t%s' % (name, '\t'.join(str(c) for c in components))
